...
export default function MagicVerify() {
  ...
  const finishEmailRedirectLogin = async () => {
    ...
    new Magic(process.env.NEXT_PUBLIC_MAGIC_PUBLISHABLE_KEY, {
      ...
      scope: ['read:email'], /* optional */
    });
    const result = await magic.oauth.getRedirectResult();

    authenticateWithServer(result.oauth.accessToken)

    alert(JSON.stringify(result, null, 2));
    // magicClient.oauth.getRedirectResult()
    //     .then((oauth) => authenticateWithServer(oauth.accessToken));
  };

  // Send token to server to validate
  const authenticateWithServer = async (didToken) => {
    let res = await fetch("/api/login-verify-oauth", {
      method: "POST",
      headers: {
